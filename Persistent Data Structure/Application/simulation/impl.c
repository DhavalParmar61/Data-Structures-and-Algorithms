...
void transit(node* state_ptr,int version,int no_transition)
{
	...
	int i=0,j=0,k=1;
	float x;
	FILE* fp=fopen("simulation(0.2,0.4,0.4).txt","w"); 
...
node* update_parentleft(list* list, node* list_ptr, node* newnode, int current_version)
{
  ...
  if(list_ptr->mod->version == unused)
  ...
  else
    {
      ...
      newnode_parent->prev = update_parentleft(list, list_ptr->mod->prev, newnode_parent, current_version);
    }
      ...
      if(index != 0)
		{
		  ...
		  newnode->prev = update_parentleft(list, list_ptr, newnode, current_version);
		}
      else
		{
	      ...
	      if(index != 0)
	       {
	      	...
	      	newnode->next = update_parentright(list, list_ptr->mod->next, newnode, current_version);
	      	newnode->prev = update_parentleft(list, list_ptr->mod->prev, newnode, current_version);
	       }
	  	 else
	      {
	       node* newnode = addNode(data,fw_prob,bw_prob,ss_prob);
   ...
   if((list_ptr->next == NULL) && (list_ptr->mod->version == unused))
	{
  		...
  		else
    	{
      		...
      		newnode->prev = update_parentleft(list, list_ptr->prev->mod->prev, newnode, current_version);
    	}
   ...
   if((list_ptr->mod->next == NULL) && (list_ptr->mod->version != unused))
	{
	  if(list_ptr->mod->prev->mod->version == unused)
	  ...
	  else
	    {
	      ...
	      newnode->prev = update_parentleft(list, list_ptr->mod->prev->mod->prev, newnode, current_version);
	    }
      ...
      if(list_ptr->mod->version == unused)
	{
	  if((list_ptr->prev->mod->version == unused) && (list_ptr->next->mod->version == unused))
	  ...
	  else
	    {
	      ...
	      newnode_prev->prev = update_parentleft(list, list_ptr->prev->mod->prev, newnode_prev, current_version);
      ...
      else
	{
	  if((list_ptr->mod->prev->mod->version == unused) && (list_ptr->mod->next->mod->version == unused))
	  ...
	  else
	    {
	      ...
	      newnode_prev->prev = update_parentleft(list, list_ptr->mod->prev->mod->prev, newnode_prev, current_version);
